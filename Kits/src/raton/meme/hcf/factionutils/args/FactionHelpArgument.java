    ...
    private void showPage(CommandSender sender, String label, int pageNumber) {
        ...
        if (pages == null) {
            ...
            for (CommandArgument argument : executor.getArguments()) {
                ...
                count++;
                pages.get(val).add(ChatColor.RED + "§7* §b/" + label + ' ' + argument.getName() + "§7: " + ChatColor.WHITE + argument.getDescription());
        ...
        sender.sendMessage("§b§lFactions Help " + "" + ChatColor.GRAY + "(Page " + pageNumber + '/' + totalPageCount + ')');
